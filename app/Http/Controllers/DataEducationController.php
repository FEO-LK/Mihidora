...
class DataEducationController extends Controller
{
 ...
 /** Filter datahub by district and tags */
    public function filterDatahub(Request $body)
    {
        ...
        $dataset = DataEducation::where('type','=',1)->select('*')->get();
        $tags = [];
        foreach($dataset as $data) {
    ...
    {   //load DataItem
        ...
        $DataItemCity = Cities::select('name_en')->where('id', $DataItem->city_id)->first();
        $DataItemDistrict = Districts::select('name_en')->where('id', $DataItem->district_id)->first();


        if($DataItemCity == null) { $city = 339; } else { $city = $DataItemCity; }
        if($DataItemDistrict == null) { $district = 26; } else { $district = $DataItemDistrict; }

        //load DataItem Organisation
        $organization = Organizations::select('org_name', 'slug', 'org_type', 'city_id')->where('id', $DataItem->organization_id)->first();
        if($organization == null or $organization->city_id == null){
            $organizationCity = Cities::select('name_en')->where('id', 340)->first(); //id 340 = none
            $organizationType = '-';
        }else{
        ...
        if($organizationCity == null || $organizationCity == 'none') { $Orgdistrict = 1; } else { $Orgdistrict = $organizationCity; }
    ...
    public function filterELearningData(Request $body)
        {
            $district_id = $body->input('district');
            $tag_names = $body->input('tags');
            $dataset = DataEducation::where('type','=',2)->select('*')->get();
            $tags = [];
            foreach($dataset as $data) {
                $tag = $data->tags->pluck('name','type','id');
                array_push($tags, $tag);
    ...
    public function ELearningMaterialProfile($slug)
    {
        ...
        //dd($profile);
        $projectCity = Cities::select('name_en')->where('id', $profile->city_id)->first();
        $ProjectDistrict = Districts::select('name_en')->where('id', $profile->district_id)->first();
    ...
    public function updateData(Request $request, $id)
    {
        ...
        if($validator->fails()){
            ...
            if($dataEdu) {
                ...
                $dataEdu->slug = strtolower(str_replace(' ', '', $request->title).uniqid());
    ...
    public function updateEducation(Request $request, $id)
    {
        ...
        if($validator->fails()){
            ...
            if($dataEdu) {
                ...
                $dataEdu->phone = $request->input('phone');
                $dataEdu->email = $request->input('email');
                $dataEdu->what_you_will_learn = json_encode($request->input('what_you_will_learn'));
