...
function ResourceSharingProfile() {

    const [resourceSharingProfile, setResourceSharingProfile] = useState([]);
    ...
    useEffect(() => {
        getResourceSharingProfile();

    }, [params.slug]);

    const getResourceSharingProfile = () => {
        axios.get(`/api/resource-exchange-resource/${params.slug}`).then(res => {
            if (res.data.status === 200) {
                setResourceSharingProfile(res.data.profile);
    ...
    useEffect(() => {
        if (resourceSharingProfile?.description) {
            editorConfig.editorState = resourceSharingProfile?.description;
            setEditorInitialConfig(editorConfig);
        }
    }, [resourceSharingProfile?.description]);
    ...
    return (
        <BaseLayout title={"Projects"}>
            ...
            <Container>
                <h3>Resource Hub - Resource Pool</h3>
                <Grid
                    container
                    direction="row"
                ...
                >
                    <Grid item sm={12} md={9} style={{padding:'.5em .5em 0em 0em'}}>
                        ...
                        >
                            <Grid md={8}>
                                <h1 style={{marginTop: 0}}>{resourceSharingProfile[0]?.title}</h1>
                            </Grid>
                            <Grid md={4} style={{marginTop:'1em'}}>
                                <Typography variant="p">
                                    Location : {district?.name_en}
                                    <span style={{display:'flex', flexDirection: 'column', fontSize:'.8em'}}>
                                        Posted {Math.round(Math.abs(new Date() - new Date(resourceSharingProfile[0]?.created_at))/(36e5*24))} days ago
                                    </span>
                                </Typography>
                            </Grid>
                        </Grid>
                        ...
                        <Grid
                            ...
                            justifyContent="space-between"
                        >
                            <Grid md={6} style={{paddingRight: '.5em'}}>
...
export default ResourceSharingProfile;
