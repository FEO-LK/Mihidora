...
function ResourceExchangeResourceSharing() {

    const [resourceSharingList, setResourceSharingList] = useState([]);
    ...
    const getResourceList = () => {
        axios.get(`/api/classified-resource-sharing`).then(res => {
            if(res.data.status === 200) {
                setResourceSharingList(res.data.classifieds);
    ...
    const getFilteredJobs = (district, tags) => {
        axios.post(
            '/api/classified-resources-filtered',
            {district, tags}
        ).then(res => {
            if(res.data.status === 200) {
                let filteredJobs;
                ...
                setResourceSharingList(filteredJobs);
            }
        })
    }
    ...
    const handleOnChangeSortBy = (e) => {
        ...
        if (e.target.value === 'A-Z') {
            setResourceSharingList(sorted(resourceSharingList))
        } else {
            if (district == '26') {
    ...
    const handleClick = (slug) => {
        navigate(`/resource-exchange-resource-sharing/${slug}`);
    }

    const [sortBy, setSortBy] = useState('Most Relevant');

    const howManyPages = Math.ceil(resourceSharingList.length / 4);

    const getProjectMapList = () => {
        axios.get(`/api/classified-resource-sharing-map`).then(res => {
            if (res.data.status === 200) {
                setProjectMapList(res.data.resource_sharing_map);
            }
        });
    }
            ...
            <div className="topic-title-section">
                <Container>
                    <Grid container>
                        <Grid item sm={12} md={6} className="title-and-description">
                            <Typography variant="h1" style={{marginBottom: 0}}>Resource Exchange / Resource
                                Sharing</Typography>
                        </Grid>
                ...
                <Container style={{display:'flex', justifyContent: 'center', marginTop:'2em', marginBottom: '2em'}}>
                    ...
                    >
                        ...
                        <a href='./grants-and-proposals'>Grants and Proposals</a>
                                ...
                                <Grid container>
                                    <Grid item sm={12} md={4}>
                                        <Typography variant="span"
                                                    className="item-count">Showing {resourceSharingList.length} Resource
                                            Sharing</Typography>
                                    </Grid>
                                </Grid>
                            </Grid>
                            ...
                            view === 'map' ?
                                ...
                                <>
                                    <Grid container spacing={2} style={{paddingLeft: '20px'}}>
                                        {resourceSharingList.slice((currentPage - 1) * 4, currentPage * 4).map((jobDetails, key) => (
...
export default ResourceExchangeResourceSharing;
