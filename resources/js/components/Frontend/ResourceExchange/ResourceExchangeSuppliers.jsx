...
function ResourceExchangeSuppliers() {
    ...
    const stringSearch = () => {
        axios.get(`/api/classified-suppliers`).then(res => {
    ...
    const getResourceList = () => {
        axios.get(`/api/classified-suppliers`).then(res => {
            if(res.data.status === 200) {
                setSupplierList(res.data.classifieds);
    ...
    const getFilteredJobs = (district, tags) => {
        axios.post(
            '/api/classified-suppliers-filtered',
            {district, tags}
        ).then(res => {
            if(res.data.status === 200) {
                let filteredJobs;
    ...
    const handleOnChangeSortBy = (e) => {
        ...
        if (e.target.value === 'A-Z') {
            setSupplierList(sorted(resourceSharingList))
        } else {
            if (district == '26') {
    ...
    const handleClick = (slug) => {
        navigate(`/resource-exchange-supplier/${slug}`);
    }

    const [sortBy, setSortBy] = useState('Most Relevant');

    const howManyPages = Math.ceil(supplierList?.length / 4);

    const getProjectMapList = () => {
        axios.get(`/api/classified-suppliers-map`).then(res => {
            if (res.data.status === 200) {
                setProjectMapList(res.data.suppliers_map);
            }
        });
    }
                                ...
                                <Grid container>
                                    <Grid item sm={12} md={4}>
                                        <Typography variant="span"
                                                    className="item-count">Showing {supplierList?.length} Suppliers</Typography>
                                    </Grid>
                                </Grid>
                            </Grid>
                        </Grid>
                            ...
                            view === 'map' ?
                                ...
                                <>
                                    <Grid container spacing={2} style={{paddingLeft: '20px'}}>
                                        {supplierList.slice((currentPage - 1) * 4, currentPage * 4).map((jobDetails, key) => (
...
export default ResourceExchangeSuppliers;
