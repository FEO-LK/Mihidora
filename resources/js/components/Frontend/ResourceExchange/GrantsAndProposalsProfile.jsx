...
function GrantsAndProposalsProfile() {
    ...
    const getProposalProfile = () => {
        axios.get(`/api/resource-exchange-proposal/${params.slug}`).then(res => {
            if (res.data.status === 200) {
                setProposalProfile(res.data.profile);
    ...
    useEffect(() => {
        if (proposalProfile?.description) {
            editorConfig.editorState = proposalProfile?.description;
            setEditorInitialConfig(editorConfig);
        }
    }, [proposalProfile?.description]);
    ...
    return (
        <BaseLayout title={"Projects"}>
            ...
            <Container>
                <h3>Resource Hub - Grants and Proposals</h3>
                <Grid
                    container
                    direction="row"
                ...
                >
                     <Grid item sm={12} md={9} style={{padding:'.5em .5em 0em 0em'}}>
                        ...
                        >
                            <Grid md={8}>
                                <h1 style={{marginTop: 0}}>{proposalProfile?.title}</h1>
                            </Grid>
                            ...
                            <Grid md={4} style={{marginTop:'1em'}}>
                                <Typography variant="p">
                                    ...
                                    <span style={{display:'flex', flexDirection: 'column', fontSize:'.8em'}}>
                                        Posted {Math.round(Math.abs(new Date() - new Date(proposalProfile?.created_at))/(36e5*24))} days ago
                                    </span>
                                </Typography>
                            </Grid>
                        </Grid>
                        {/* <p>Posted on {postedDateFormat(jobProfile?.created_at)}</p> */}
                        <Grid
                            container
                            direction="row"
                            justifyContent="space-between"
...
export default GrantsAndProposalsProfile;
