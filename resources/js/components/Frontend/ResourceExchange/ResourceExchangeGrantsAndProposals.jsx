...
function ResourceExchangeGrantsAndProposals() {
    ...
    const handleClick = (slug) => {
        navigate(`/resource-exchange-proposal/${slug}`);
    }
    ...
    const getProjectMapList = () => {
        axios.get(`/api/classified-proposals-map`).then(res => {
            if (res.data.status === 200) {
                setProjectMapList(res.data.Proposals_map);
            }
        });
    }
...
export default ResourceExchangeGrantsAndProposals;
